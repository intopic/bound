...
/** NEXT_PUBLIC_BOUND_FEE_BPS: whole basis points, 0 to 100 (30 = 0.3%); 30 when unset or empty. */
export function feeBpsSetting(raw: string | undefined): bigint {
  const value = raw?.trim() ?? '';
  if (value === '') return 30n;
  if (!/^\d{1,3}$/.test(value) || BigInt(value) > 100n) {
    throw new Error(`NEXT_PUBLIC_BOUND_FEE_BPS must be a whole number of basis points from 0 to 100 (30 means 0.3%), not "${raw}".`);
  }
  return BigInt(value);
}

/** NEXT_PUBLIC_BOUND_TREASURY: a Solana address, or empty for test mode (no fee). */
export function treasurySetting(raw: string | undefined): string | null {
  const value = raw?.trim() ?? '';
  if (value === '') return null;
  if (!isAddress(value)) throw new Error(`NEXT_PUBLIC_BOUND_TREASURY must be a Solana address, or empty for test mode: "${raw}" is not one.`);
  return value;
}
...
/** BOUND_MAX_NETWORK_FEE_LAMPORTS: lamports, at least two signatures' worth; 500,000 when unset or empty. */
export function maxNetworkFeeSetting(raw: string | undefined): bigint {
  ...
  if (!/^\d{1,19}$/.test(value) || BigInt(value) < LEAST_NETWORK_FEE_LAMPORTS) {
    throw new Error(`BOUND_MAX_NETWORK_FEE_LAMPORTS must be a whole number of lamports of at least ${LEAST_NETWORK_FEE_LAMPORTS}, not "${raw}".`);
  }
  return BigInt(value);
}
...
export function checkDeploymentSettings(env: Record<string, string | undefined>): void {
  const fee = feeBpsSetting(env.NEXT_PUBLIC_BOUND_FEE_BPS);
  const treasury = treasurySetting(env.NEXT_PUBLIC_BOUND_TREASURY);
  maxNetworkFeeSetting(env.BOUND_MAX_NETWORK_FEE_LAMPORTS);
  if (treasury && fee === 0n) {
    throw new Error('NEXT_PUBLIC_BOUND_FEE_BPS is 0 while NEXT_PUBLIC_BOUND_TREASURY is set: every swap would be refused. Set the fee (30 = 0.3%), or empty the treasury for test mode.');
  }
}
