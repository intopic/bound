...
var BoundOrderError = class extends Error {
	id;
	record;
	...
	if (command === "prepare") {
		...
		if (prior && (prior.state === "confirmed" || prior.state === "pending")) return {
			...
			output: {
				...
				error: prior.state === "confirmed" ? "This order already swapped. Nothing new was prepared." : "This order has a transaction that may still land: run `bound-verify recover`. Nothing new was prepared."
			}
		};
		...
		try {
			...
			if (waiting.length) return {
				...
				output: {
					...
					sent: false,
					pending: waiting,
					error: "An earlier swap from this wallet may still land: run `bound-verify recover` first. Nothing was sent."
				}
			};
			const problems = await checkPrepared(prepared, intent, deps.rpc, { requestTimeoutMs: deps.requestTimeoutMs });
