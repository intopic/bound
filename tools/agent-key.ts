 ...
 *   node tools/agent-key.ts --key-secret   a new ORIENTIM_KEY_SECRET for self-serve keys (moving the
 *                                      old one to ORIENTIM_KEY_SECRET_PREVIOUS keeps its keys working)
 */
...
if (arg === '--secret') {
...
} else if (arg === '--key-secret') {
  console.log(`ORIENTIM_KEY_SECRET=${randomBytes(32).toString('base64')}`);
  ...
  console.error('usage: node tools/agent-key.ts <id> | --secret | --key-secret   (id: letters, digits, _ or -, up to 40)');
  process.exit(2);
}
